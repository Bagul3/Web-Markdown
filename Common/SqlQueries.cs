...
WHERE (((Left([T2_LOOK]![KEY],3))='CAT'))
		...
		public static string AllSkusQuery =>
			@"SELECT ([T2_BRA].[REF] + [F7]) AS NEWSTYLE, Suppliers.MasterSupplier, Dept.MasterDept, Colour.MasterColour, Colour.F7, T2_HEAD.SHORT, T2_HEAD.MINSIZE, T2_HEAD.MAXSIZE,T2_HEAD.REM, T2_HEAD.REM2,
...
WHERE (((Left([T2_LOOK]![KEY],3))='CAT'))
                                   ...
                                   WHERE [T2_HEAD].SELL <> [T2_HEAD].SELLB AND [T2_HEAD].SELLB <> 0 AND [T2_HEAD].SELL <> 0
									GROUP BY ([T2_BRA].[REF] + [F7]), Suppliers.MasterSupplier, Dept.MasterDept, Colour.MasterColour, Colour.F7, T2_HEAD.USER1, T2_HEAD.DESC, T2_HEAD.MINSIZE, T2_HEAD.MAXSIZE,
									 T2_HEAD.SHORT, T2_HEAD.[DESC], T2_HEAD.[GROUP], T2_HEAD.STYPE, T2_HEAD.SIZERANGE, T2_HEAD.SUPPLIER, T2_HEAD.SUPPREF,T2_HEAD.REM, T2_HEAD.REM2,
